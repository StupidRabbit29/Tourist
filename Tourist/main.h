...
enum Status { ERROR, OK, STACK_FULL, STACK_EMPTY, UNABLE };//程序状态
enum Travel_Strategy { STRA_minCOST, STRA_minTIME, STRA_limTIME_minCOST };//旅行策略
enum Location { IN_CAR, IN_TRAIN, IN_AIRPLANE, STAY_IN_CITY, ARRIVE };//旅客状态
...
}trans_t_Node, *Ptr_trans_t_Node;

/*边 结构*/
typedef struct Edge {
	struct trans_t *p_TransTable;//此边上的航班表
...
/*栈 结构*/
typedef struct {
	int *base, *top, stack_size;//栈底、栈顶、栈大小
}SqStack;

/*系统时间 结构*/
typedef struct SYSTEM_TIME {
	int year, month, date, hour;//年、月、日、小时
}SYSTEM_TIME;
...
typedef struct passenger {
	...
	int pass_by[2][MAX_NODE_NUM];//row0要求途经的城市;row1在该地停留时间
